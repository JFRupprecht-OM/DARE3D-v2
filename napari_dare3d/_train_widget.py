...
@magic_factory(
    ...
    epochs={"min": 1, "tooltip": "Max training epochs per model."},
    batch_size={"min": 1, "tooltip": "3D patches per step. Lower if you hit GPU out-of-memory."},
    cell_radius={"min": 1, "tooltip": "Radius (voxels) of the segmentation target spheres."},
    seg_crop_size={"min": 16, "tooltip": "3D training patch size for segmentation."},
    train_segmentation={"tooltip": "Train the segmentation (division-center) model."},
    train_regression={"tooltip": "Train the regression (division-axis) model."},
    run_eval={"tooltip": "After training, run dare3d eval.py on the val set (needs both models + GT)."},
    threshold={"min": 0.0, "max": 1.0, "step": 0.05, "tooltip": "Segmentation probability threshold used at evaluation."},
